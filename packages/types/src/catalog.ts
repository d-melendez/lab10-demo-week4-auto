...
export const TOOL_CATALOG: ToolDefinition[] = [
  ...
  },
  {
    ...
    description:
      "Lists the user's Google calendars (calendar list). Read-only; does not require confirmation.",
    risk: "low",
    requires_integration: "google_calendar",
    parameters_schema: {
      type: "object",
      properties: {
        max_results: { type: "number", description: "Max calendars to return (default 50, max 250)" },
      },
      required: [],
    },
  ...
  },
  {
    ...
    description:
      "Creates a new secondary Google Calendar for the user. Requires human confirmation before execution.",
    risk: "medium",
    requires_integration: "google_calendar",
    parameters_schema: {
      type: "object",
      properties: {
        summary: { type: "string", description: "Calendar title (required)" },
        description: { type: "string", description: "Optional description" },
        timeZone: { type: "string", description: "IANA timezone (e.g. America/Bogota). Optional." },
      },
      required: ["summary"],
    },
    displayName: "Google Calendar: crear calendario",
    displayDescription: "Crea un calendario nuevo (requiere confirmación).",
  },
  {
    id: "calendar_delete_calendar",
    name: "calendar_delete_calendar",
    description:
      "Permanently deletes a secondary Google Calendar by id. Cannot delete the primary calendar. Requires human confirmation.",
    risk: "medium",
    requires_integration: "google_calendar",
    parameters_schema: {
      type: "object",
      properties: {
        calendarId: {
          type: "string",
          description: "Calendar id from calendar_list_calendars (e.g. abc@group.calendar.google.com).",
        },
      },
      required: ["calendarId"],
    },
    displayName: "Google Calendar: borrar calendario",
    displayDescription: "Elimina un calendario secundario (requiere confirmación).",
  },
  {
    id: "read_file",
    name: "read_file",
