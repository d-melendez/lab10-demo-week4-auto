...
export async function executeGoogleCalendarTool(
  ...
  switch (toolName) {
    case "calendar_list_calendars": {
      const max = Math.min(250, Math.max(1, (args.max_results as number) || 50));
      const data = await calendarFetch(
        accessToken,
        `/users/me/calendarList?maxResults=${max}&showDeleted=false`
      );
    ...
    case "calendar_create_calendar": {
      ...
      const created = await calendarFetch(accessToken, "/calendars", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    case "calendar_delete_calendar": {
      ...
      await calendarFetch(accessToken, `/calendars/${calendarId}`, { method: "DELETE" });
      return { message: "Calendar deleted", calendarId: args.calendarId };
    }
    default:
...
export const TOOL_HANDLERS: ToolHandlers = {
  ...
  calendar_list_calendars: async (input, ctx) =>
    executeGoogleCalendarTool("calendar_list_calendars", input, ctx.googleCalendarAccessToken!),

  calendar_create_calendar: async (input, ctx) =>
    executeGoogleCalendarTool("calendar_create_calendar", input, ctx.googleCalendarAccessToken!),

  calendar_delete_calendar: async (input, ctx) =>
    executeGoogleCalendarTool("calendar_delete_calendar", input, ctx.googleCalendarAccessToken!),
